...
class TestVisualizationOutputs:
    ...
    def test_plot_greek_vs_time(self, model, T_range):
        for greek_name in ("price", "delta", "gamma", "vega", "theta", "rho"):
            fig, ax = plot_greek_vs_time(model, greek_name, S=100, K=100, r=0.05, T_range=T_range)
            assert isinstance(fig, matplotlib.figure.Figure)
            plt.close(fig)
    ...
    def test_plot_vol_sensitivity(self):
        fig, ax = plot_vol_sensitivity(
            BlackScholesModel, S=100, K=100, T=1.0, r=0.05, sigma_range=np.linspace(0.05, 0.80, 50)
        )
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
...
class TestNewVisualizations:
    def test_plot_implied_vol_smile(self, model):
        ...
        market_prices = [{"K": k, "price": model.price(100, k, 1.0, 0.05, "call")} for k in strikes]
    ...
    def test_plot_greeks_heatmap_delta(self, model):
        ...
        fig, ax = plot_greeks_heatmap(model, "delta", S_range, T_range, K=100, r=0.05)
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
    ...
    def test_plot_greeks_heatmap_gamma(self, model):
        S_range = np.linspace(70, 130, 30)
        T_range = np.linspace(0.05, 2.0, 20)
        fig, ax = plot_greeks_heatmap(model, "gamma", S_range, T_range, K=100, r=0.05)
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
    ...
    def test_plot_greeks_heatmap_vanna(self, model):
        S_range = np.linspace(70, 130, 30)
        T_range = np.linspace(0.05, 2.0, 20)
        fig, ax = plot_greeks_heatmap(model, "vanna", S_range, T_range, K=100, r=0.05)
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
...
class TestMCVisualizations:
    ...
    def test_plot_variance_reduction_comparison(self):
        ...
        results = {
            ...
            "Antithetic": mc.price_european(100, 100, 1.0, 0.05, 0.20, antithetic=True),
            "Control": mc.price_european(100, 100, 1.0, 0.05, 0.20, control_variate=True),
        }
...
class TestExoticVisualizations:
    ...
    def test_plot_exotic_payoff_asian(self):
        ...
        fig, axes = plot_exotic_payoff(
            asian, S0=100, T=1.0, r=0.05, sigma=0.20, n_paths_show=10, n_steps=50, seed=42
        )
        assert isinstance(fig, matplotlib.figure.Figure)
        assert axes.shape == (2,)
        plt.close(fig)
    ...
    def test_plot_exotic_payoff_barrier(self):
        ...
        fig, axes = plot_exotic_payoff(
            barrier, S0=100, T=1.0, r=0.05, sigma=0.20, n_paths_show=10, n_steps=50, seed=42
        )
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
    ...
    def test_plot_exotic_payoff_lookback(self):
        ...
        fig, axes = plot_exotic_payoff(
            lookback, S0=100, T=1.0, r=0.05, sigma=0.20, n_paths_show=10, n_steps=50, seed=42
        )
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
    ...
    def test_plot_exotic_payoff_digital(self):
        ...
        fig, axes = plot_exotic_payoff(
            digital, S0=100, T=1.0, r=0.05, sigma=0.20, n_paths_show=10, n_steps=50, seed=42
        )
        assert isinstance(fig, matplotlib.figure.Figure)
        plt.close(fig)
    ...
    def test_plot_exotic_payoff_invalid_n_paths_show_raises(self):
        ...
        with pytest.raises(ValueError, match="n_paths_show"):
            plot_exotic_payoff(digital, S0=100, T=1.0, r=0.05, sigma=0.20, n_paths_show=0)
