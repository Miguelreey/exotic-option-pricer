...
class TestPutCallParity:
    ...
    @pytest.mark.parametrize("S,K,T,r", [
        (100, 100, 1.0, 0.05), (100, 80, 0.5, 0.03),
        (100, 120, 2.0, 0.08), (50, 50, 0.25, 0.01),
        (200, 100, 0.01, 0.10), (100, 100, 5.0, 0.02),
        (100, 100, 1.0, 0.0),  (150, 130, 0.25, 0.02),
    ])
    ...
    @pytest.mark.parametrize("S,K,T,r,q", [
        (100, 100, 1.0, 0.05, 0.02), (100, 80, 0.5, 0.03, 0.05),
...
class TestGreekSymmetry:
    ...
    @pytest.mark.parametrize("S,K,T,r", [
        (100, 100, 1.0, 0.05), (120, 100, 0.5, 0.03),
        (80, 100, 2.0, 0.08),  (50, 100, 0.1, 0.01),
    ])
    ...
    @pytest.mark.parametrize("S,K,T,r", [
        (100, 100, 1.0, 0.05), (120, 100, 0.5, 0.03), (80, 100, 2.0, 0.08),
    ])
    ...
    @pytest.mark.parametrize("S,K,T,r,q", [
        (100, 100, 1.0, 0.05, 0.02), (120, 100, 0.5, 0.03, 0.04),
    ])
...
class TestGreeksFiniteDifference:
    ...
    def test_delta_fd(self, bs, params):
        h = 0.01
        S = params['S']
        fd = (bs.price(S+h, params['K'], params['T'], params['r'], 'call')
              - bs.price(S-h, params['K'], params['T'], params['r'], 'call')) / (2*h)
    ...
    def test_gamma_fd(self, bs, params):
        ...
        fd = (bs.price(S+h, params['K'], params['T'], params['r'], 'call')
              ...
              + bs.price(S-h, params['K'], params['T'], params['r'], 'call')) / h**2
        assert abs(fd - bs.gamma(**params, option_type='call')) < 1e-4

    def test_vega_fd(self, bs, params):
        h = 0.0001
        fd = (BlackScholesModel(sigma=bs.sigma+h).price(**params, option_type='call')
              - BlackScholesModel(sigma=bs.sigma-h).price(**params, option_type='call')) / (2*h)
        assert abs(fd - bs.vega(**params, option_type='call')) < 0.01

    def test_theta_fd(self, bs, params):
        h = 1.0 / 365.0
        T = params['T']
        fd = (bs.price(params['S'], params['K'], T-h, params['r'], 'call')
              - bs.price(params['S'], params['K'], T, params['r'], 'call')) / h
        assert abs(fd - bs.theta(**params, option_type='call')) < 0.05

    def test_rho_fd(self, bs, params):
        h = 0.0001
        fd = (bs.price(params['S'], params['K'], params['T'], params['r']+h, 'call')
              - bs.price(params['S'], params['K'], params['T'], params['r']-h, 'call')) / (2*h)
...
class TestBSPDE:
    ...
    @pytest.mark.parametrize("S,K,T,r,otype", [
        (100, 100, 1.0, 0.05, 'call'), (100, 100, 1.0, 0.05, 'put'),
        (120, 100, 0.5, 0.03, 'call'), (80, 100, 2.0, 0.08, 'put'),
        (50, 50, 0.25, 0.01, 'call'),
    ])
    def test_pde(self, bs, S, K, T, r, otype):
        V = bs.price(S, K, T, r, otype)
        lhs = (bs.theta(S, K, T, r, otype) + r * S * bs.delta(S, K, T, r, otype)
               + 0.5 * bs.sigma**2 * S**2 * bs.gamma(S, K, T, r, otype))
        assert abs(lhs - r * V) < 1e-10

    @pytest.mark.parametrize("S,K,T,r,q,otype", [
        (100, 100, 1.0, 0.05, 0.03, 'call'), (100, 100, 1.0, 0.05, 0.03, 'put'),
    ])
...
class TestHomogeneity:
    ...
    def test_call_homogeneity(self, bs, scale):
        ...
        assert np.isclose(bs.price(scale*HULL_S, scale*HULL_K, HULL_T, HULL_R, 'call'),
                          scale * base, rtol=1e-12)

    @pytest.mark.parametrize("scale", [0.5, 2.0, 10.0])
    def test_put_homogeneity(self, bs, scale):
        base = bs.price(HULL_S, HULL_K, HULL_T, HULL_R, 'put')
        assert np.isclose(bs.price(scale*HULL_S, scale*HULL_K, HULL_T, HULL_R, 'put'),
                          scale * base, rtol=1e-12)
...
class TestPriceBounds:
    ...
    @pytest.mark.parametrize("S,K,T,r", [
        (100, 100, 1.0, 0.05), (120, 80, 0.5, 0.03), (80, 120, 2.0, 0.08),
    ])
...
class TestSecondOrderGreeks:
    ...
    def test_vanna_fd(self, bs):
        """Vanna = dVega/dS."""
        h = 0.01
        fd = (bs.vega(self.S+h, self.K, self.T, self.r, 'call')
              - bs.vega(self.S-h, self.K, self.T, self.r, 'call')) / (2*h)
    ...
    def test_charm_fd(self, bs):
        ...
        h = 1.0 / 365.0
        T = self.T
        fd = (bs.delta(self.S, self.K, T-h, self.r, 'call')
              - bs.delta(self.S, self.K, T, self.r, 'call')) / h
        assert abs(fd - bs.charm(self.S, self.K, T, self.r, 'call')) < 0.01

    def test_speed_fd(self, bs):
        """Speed = dGamma/dS."""
        h = 0.01
        fd = (bs.gamma(self.S+h, self.K, self.T, self.r, 'call')
              - bs.gamma(self.S-h, self.K, self.T, self.r, 'call')) / (2*h)
    ...
    def test_color_fd(self, bs):
        ...
        h = 1.0 / 365.0
        T = self.T
        fd = -(bs.gamma(self.S, self.K, T-h, self.r, 'call')
               - bs.gamma(self.S, self.K, T, self.r, 'call')) / h
    ...
    def test_vanna_same_for_call_put(self, bs):
        assert np.isclose(
            ...
            bs.vanna(self.S, self.K, self.T, self.r, 'put'), rtol=1e-12
        )
...
class TestDualGreeks:
    ...
    def test_dual_delta_fd(self, bs):
        """Dual delta = dV/dK."""
        h = 0.01
        fd = (bs.price(self.S, self.K+h, self.T, self.r, 'call')
              - bs.price(self.S, self.K-h, self.T, self.r, 'call')) / (2*h)
    ...
    def test_dual_gamma_fd(self, bs):
        ...
        fd = (bs.price(self.S, self.K+h, self.T, self.r, 'call')
              ...
              + bs.price(self.S, self.K-h, self.T, self.r, 'call')) / h**2
