...
class TestStrategyGreeks:
    def test_straddle_delta_neutral_atm(self, model, base_params):
        ...
        S_fwd = K * np.exp(-r * T)
        delta_straddle = model.delta(S_fwd, K, T, r, "call") + model.delta(S_fwd, K, T, r, "put")
    ...
    def test_straddle_short_theta(self, model, base_params):
        ...
        S, K, T, r = (base_params[k] for k in ("S", "K", "T", "r"))
        theta_straddle = model.theta(S, K, T, r, "call") + model.theta(S, K, T, r, "put")
    ...
    def test_bull_spread_pnl_bounded(self, model, base_params):
        ...
        payoff = np.maximum(S_range - K1, 0.0) - np.maximum(S_range - K2, 0.0)
        assert np.all(payoff >= -1e-14)
    ...
    def test_greeks_linear_additivity(self, model, base_params):
        ...
        def portfolio_value(spot):
            return qty_call * model.price(spot, K_call, T, r, "call") + qty_put * model.price(
                spot, K_put, T, r, "put"
            )
        ...
        analytical_delta = qty_call * model.delta(S, K_call, T, r, "call") + qty_put * model.delta(
            S, K_put, T, r, "put"
        )
    ...
    def test_strangle_cheaper_than_straddle(self, model, base_params):
        ...
        S, T, r = base_params["S"], base_params["T"], base_params["r"]
        straddle = model.price(S, 100.0, T, r, "call") + model.price(S, 100.0, T, r, "put")
        strangle = model.price(S, 105.0, T, r, "call") + model.price(S, 95.0, T, r, "put")
