...
def save_figure(fig: plt.Figure, filepath: str, dpi: int = 150) -> None:
    ...
    try:
        ...
        fig.savefig(
            str(output_path), dpi=dpi, bbox_inches="tight", facecolor="white", edgecolor="none"
        )
    finally:
        plt.close(fig)


def plot_bs_price_and_intrinsic(
    model, S_range: np.ndarray, K: float, T: float, r: float, option_type: str = "call"
    ...
    ax.plot(S_range, bs_price, color=color_main, linewidth=2.2, label="BS Price", zorder=3)
    ax.plot(
        S_range,
        intrinsic,
    ...
    ax.fill_between(S_range, intrinsic, bs_price, alpha=0.12, color=color_main, label="Time Value")
    ax.axvline(
        x=K,
        color=COLORS["atm_line"],
        linestyle="-.",
        linewidth=0.9,
        alpha=0.6,
        label=f"ATM (K={K})",
...
def plot_all_greeks(
    model, S_range: np.ndarray, K: float, T: float, r: float
    ...
    for idx, (greek_name, title_str) in enumerate(panel_config):
        ...
        ax.plot(
            S_range,
            greeks_data["put"][greek_name],
            color=COLORS["put"],
            linewidth=1.8,
            label="Put",
            linestyle="--",
        )
        ax.axvline(x=K, color=COLORS["atm_line"], linestyle="-.", linewidth=0.8, alpha=0.5)
...
def plot_greek_vs_time(
    model, greek_name: str, S: float, K: float, r: float, T_range: np.ndarray
) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    if greek_name_lower not in greek_methods:
        raise ValueError(
            f"greek_name must be one of {list(greek_methods.keys())}. Got: '{greek_name}'"
        )
    ...
    if greek_name_lower in ("gamma", "vega"):
        ...
        ax.plot(
            ...
            linewidth=2.0,
            linestyle="--",
            label=f"{greek_name_lower.capitalize()} (Put)",
        )
...
def plot_payoff_diagram(legs: List[Dict], S_range: np.ndarray) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    ax.fill_between(
        ...
        pnl,
        0,
        where=(pnl >= 0),  # type: ignore[arg-type]
        alpha=0.15,
        color=COLORS["pnl_pos"],
        interpolate=True,
    )
...
def plot_portfolio_greeks(
    legs: List[Dict], S_range: np.ndarray, model, T: float, r: float
) -> Tuple[plt.Figure, np.ndarray]:
    """
    ...
    fig.suptitle(
        f"Portfolio Greeks ($T={T}$y, $\\sigma={model.sigma * 100:.0f}\\%$)", fontsize=14, y=1.01
    )
    fig.tight_layout()
    return fig, axes
...
def plot_pnl_attribution(
    pnl_components: Dict[str, float], labels: Optional[List[str]] = None
) -> Tuple[plt.Figure, plt.Axes]:
    """
...
def plot_gamma_theta_tradeoff(
    model, S: float, r: float, K_values: np.ndarray, T_values: np.ndarray
) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    ax.set_title(
        f"Gamma-Theta Tradeoff ($S={S}$, $\\sigma={model.sigma * 100:.0f}\\%$)", fontsize=13
    )
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    return fig, ax
...
def plot_implied_vol_smile(
    ...
    ax.axvline(
        x=1.0, color=COLORS["atm_line"], linestyle="-.", linewidth=0.9, alpha=0.6, label="ATM"
    )
...
def plot_greeks_heatmap(
    ...
    ax.axhline(y=K, color="white", linestyle="--", linewidth=1.0, alpha=0.7, label=f"ATM ($K={K}$)")
...
def plot_mc_paths(
    ...
    ax.plot(t_grid, mean_path, color=COLORS_MC["mean"], linewidth=2.2, label="Mean path", zorder=5)
    ...
    ax.fill_between(
        t_grid, lower, upper, color=COLORS_MC["ci_fill"], alpha=0.12, label=r"95% distribution band"
    )

    ax.set_xlabel("Time (years)", fontsize=12)
    ax.set_ylabel("Price ($S$)", fontsize=12)
    ax.set_title(
        title or f"Monte Carlo GBM Paths ($N={n_paths:,}$, $S_0={paths[0, 0]:.0f}$)", fontsize=13
    )
...
def plot_mc_convergence(
    ...
    if errors is not None and len(errors) != len(path_counts):
        raise ValueError(
            f"errors must match path_counts in length, got {len(errors)} vs {len(path_counts)}."
        )
...
def plot_variance_reduction_comparison(
    ...
    bar_colors = [color_map.get(label.lower(), COLORS_MC["plain"]) for label in labels]
    ...
    if len(se_values) > 1:
        ...
        for i, (bar, se) in enumerate(zip(bars, se_values)):
            ...
            if i > 0:
                ax.text(
                    bar.get_width() + max(se_values) * 0.02,
...
def plot_exotic_payoff(
    ...
    ax_paths.plot(
        t_grid,
        np.mean(paths[display_idx], axis=0),
        color=COLORS_EXOTIC["mean"],
        linewidth=2.2,
    ...
    if barrier is not None:
        ax_paths.axhline(
            barrier,
            color=COLORS_EXOTIC["barrier"],
            linewidth=1.8,
            linestyle="-.",
            label=f"Barrier $H={barrier:g}$",
            zorder=4,
        )
    ...
    if isinstance(exotic, LookbackOption):
        ...
        ax_paths.plot(
            t_grid,
            running_max,
            color=COLORS_EXOTIC["max"],
            linewidth=1.4,
            linestyle=":",
            alpha=0.85,
            label="Running max (sample)",
        )
        ax_paths.plot(
            t_grid,
            running_min,
            color=COLORS_EXOTIC["min"],
            linewidth=1.4,
            linestyle=":",
            alpha=0.85,
            label="Running min (sample)",
        )
    ...
    if positive.size > 0:
        ax_hist.hist(
            positive,
            bins=40,
            color=COLORS_EXOTIC["payoff_fill"],
            edgecolor="white",
            alpha=0.85,
        )
    ...
    ax_hist.set_title(
        f"Terminal payoff distribution (P[payoff=0] = {zero_frac:.1%}, N = {n_paths_hist:,})",
        fontsize=12,
    )
